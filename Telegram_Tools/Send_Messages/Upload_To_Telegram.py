...
video_directory_path = 'C:\\Users\\sures\\Downloads\\Videos\\less_than_1MB'
...
files_directory_path = 'C:\\Users\\sures\\Downloads\\pictures\\less_than_30MB'
...
# Setup separate logger for successful uploads
success_log_file = f'successful_uploads_{timestamp}.log'
success_handler = logging.FileHandler(success_log_file, encoding='utf-8')  # Specify UTF-8 encoding
success_handler.setLevel(logging.INFO)
success_formatter = logging.Formatter('%(asctime)s - %(message)s')
success_handler.setFormatter(success_formatter)

# Create a separate logger for successes
            ...
            if mime_type and mime_type.startswith('image'):
                if file_size < 2 * 1024 * 1024:
                    with open(file_path, 'rb') as file:
                        ...
                        photo_file_paths.append(file_path)  # Store full path
                        if len(photo_group) == 10:
                            await send_photos(bot, photo_group, photo_file_paths)
                            photo_group = []
                            photo_file_paths = []
        ...
        if os.path.isfile(file_path):
            ...
            if mime_type and mime_type.startswith('video') and file_size < 50 * 1024 * 1024:  # Ensure file size is less than 50MB
                with open(file_path, 'rb') as file:
                    ...
                    video_file_paths.append(file_path)  # Store full path
                    if len(video_group) == 10:  # Upload 10 videos at a time
                        await send_videos(bot, video_group, video_file_paths)
                        video_group = []
                        video_file_paths = []
                        await asyncio.sleep(2)
    ...
    for filename in os.listdir(files_directory_path):
        ...
        if os.path.isfile(file_path):
            ...
            if mime_type and mime_type.startswith(('image', 'video')):
                file_group.append(file_path)  # Store full path
                if len(file_group) == 10:  # Upload 10 files at a time
                    await send_files(bot, file_group)
                    file_file_paths.extend(file_group)
                    file_group = []
                    await asyncio.sleep(2)
...
async def send_photos(bot, photo_group, file_paths):
    try:
        await bot.send_media_group(
            chat_id=photo_channel_id,
            media=photo_group,
            connect_timeout=5200,
            write_timeout=5200,
            read_timeout=5200
        )
        logging.info(f"Successfully uploaded a group of {len(photo_group)} photos: {', '.join(os.path.basename(path) for path in file_paths)}")
        for path in file_paths:
            success_logger.info(path)  # Log the full path
    except RetryAfter as e:
        logging.warning(f"Rate limit exceeded. Retrying in {e.retry_after} seconds. Failed files: {', '.join(os.path.basename(path) for path in file_paths)}")
        await asyncio.sleep(e.retry_after)
        await send_photos(bot, photo_group, file_paths)
    except TelegramError as e:
        logging.error(f"Failed to upload photo group: {', '.join(os.path.basename(path) for path in file_paths)}. Error: {str(e)}")

async def send_videos(bot, video_group, file_paths):
    try:
        await bot.send_media_group(
        ...
        logging.info(f"Successfully uploaded a group of {len(video_group)} videos: {', '.join(os.path.basename(path) for path in file_paths)}")
        for path in file_paths:
            success_logger.info(path)  # Log the full path
    except RetryAfter as e:
        logging.warning(f"Rate limit exceeded. Retrying in {e.retry_after} seconds. Failed files: {', '.join(os.path.basename(path) for path in file_paths)}")
        await asyncio.sleep(e.retry_after)
        await send_videos(bot, video_group, file_paths)
    except TelegramError as e:
        logging.error(f"Failed to upload video group: {', '.join(os.path.basename(path) for path in file_paths)}. Error: {str(e)}")

async def send_files(bot, file_group):
